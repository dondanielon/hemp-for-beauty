...
router.get("/info", authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (!userInfo) return res.status(404).json({ msg: "email not found" });
...
router.post("/create", async (req, res, next) => {
  ...
  try {
    ...
    if (emailInUse)
      return res.status(200).json({ msg: "email en uso", content: null });
    ...
    return res
      .status(201)
      .json({ msg: "usuario creado", content: newUser.email });
  } catch (error) {
    next(error);
  }
});
...
router.post("/login", async (req, res, next) => {
  ...
  try {
    ...
    if (!user)
      return res
        .status(401)
        .json({ msg: "email no registrado", content: null });
    ...
    if (passwordMatch) {
      ...
      return res
        .status(401)
        .json({ msg: "contraseña incorrecta", content: null });
    }
  } catch (error) {
    next(error);
  }
...
router.get("/all", authMiddleware, async (req, res, next) => {
  ...
  if (role !== ADMIN_ROLE)
    return res
      .status(401)
      .json({ msg: "unauthorized missing permissions", content: null });
  try {
