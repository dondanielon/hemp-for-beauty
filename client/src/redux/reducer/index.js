...
export default function rootReducer(state = initialState, { type, payload }) {
  switch (type) {
    ...
    case ADD_TO_CART:
      ...
      window.localStorage.setItem("hempCartList", JSON.stringify(list));
      window.localStorage.setItem(
        "expiration-date-cart-list",
    ...
    case ADD_SPECIFIC_TO_CART:
      const newList = state.cartList.map((item) =>
        item.id === payload.id
          ? { ...item, qty: item.qty + payload.quantity }
          : item
      );
      const expiration = new Date();
      expiration.setDate(expiration.getDate() + 2);

      window.localStorage.setItem("hempCartList", JSON.stringify(newList));
      window.localStorage.setItem(
        "expiration-date-cart-list",
    ...
    case REMOVE_SPECIFIC_TO_CART:
      ...
      window.localStorage.setItem("hempCartList", JSON.stringify(updatedList));
      window.localStorage.setItem(
        "expiration-date-cart-list",
        exptnDate.toISOString()
      );
